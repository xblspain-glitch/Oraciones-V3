...
function backupComparableStateV31268(value){
  try{
    ...
    /* V3.1.305: antes se guardaba aquí otra copia completa de todos los datos.
...
function toggleReadingUI(){
  ...
  const wasAtPageTop=(root?.scrollTop||window.scrollY||0)<=48;
  document.body.classList.toggle("hide-reading-ui");
  if(!willHide||!wasAtPageTop)return;
...
const APP_VERSION_LABEL = "v3.1.320";
const APP_VERSION_ZIP = "Oraciones_V3.1.320_TARJETAS_MILAGROS_ACTUALIZADAS.zip";
...
const APP_VERSION_V31249 = "3.1.320";
...
const DRIVE_BACKUP_FOLDER_V31296='Oraciones - Backups';
const DRIVE_FOLDER_ID_KEY_V31296='oracionesDriveBackupFolderIdV31296';
const DRIVE_LAST_BACKUP_KEY_V31296='oracionesLastDriveBackupAtV31296';
let driveAccessTokenV31296='';
let driveAccessTokenExpiresAtV31296=0;
function driveStampV31296(){return new Date().toISOString().slice(0,19).replace(/[:T]/g,'-');}
function setDriveBackupStatusV31296(message,isError=false){const el=document.getElementById('driveBackupStatus');if(!el)return;el.textContent=message||'';el.classList.toggle('error',!!isError);}
function refreshDriveBackupStatusV31296(){
  const raw=localStorage.getItem(DRIVE_LAST_BACKUP_KEY_V31296);
  if(!raw){setDriveBackupStatusV31296(`Destino: Mi unidad / ${DRIVE_BACKUP_FOLDER_V31296}`);return;}
  ...
  setDriveBackupStatusV31296(formatted?`Última copia en Drive: ${formatted}`:`Destino: Mi unidad / ${DRIVE_BACKUP_FOLDER_V31296}`);
}
function requestDriveAccessTokenV31296(forceAccountSelection=false){
  if(!forceAccountSelection&&driveAccessTokenV31296&&Date.now()<driveAccessTokenExpiresAtV31296-30000)return Promise.resolve(driveAccessTokenV31296);
  if(forceAccountSelection){driveAccessTokenV31296='';driveAccessTokenExpiresAtV31296=0;}
  return new Promise((resolve,reject)=>{
    ...
    const tokenClient=google.accounts.oauth2.initTokenClient({client_id:DRIVE_CLIENT_ID_V31296,scope:DRIVE_SCOPE_V31296,callback:response=>{
      ...
      driveAccessTokenV31296=response.access_token;driveAccessTokenExpiresAtV31296=Date.now()+(Math.max(60,Number(response.expires_in)||3600)*1000);resolve(driveAccessTokenV31296);
...
async function driveFetchV31296(url,options={}){const token=await requestDriveAccessTokenV31296();const headers=new Headers(options.headers||{});headers.set('Authorization',`Bearer ${token}`);const response=await fetch(url,{...options,headers});if(response.status===401){driveAccessTokenV31296='';driveAccessTokenExpiresAtV31296=0;}if(!response.ok){let detail='';try{detail=(await response.json())?.error?.message||''}catch(_){ }throw new Error(detail||`Google Drive respondió con error ${response.status}`);}return response;}
async function getStoredDriveFolderV31296(){const id=localStorage.getItem(DRIVE_FOLDER_ID_KEY_V31296);if(!id)return null;try{const r=await driveFetchV31296(`https://www.googleapis.com/drive/v3/files/${encodeURIComponent(id)}?fields=id,name,mimeType,trashed`);const f=await r.json();if(!f.trashed&&f.mimeType==='application/vnd.google-apps.folder')return f;}catch(_){ }localStorage.removeItem(DRIVE_FOLDER_ID_KEY_V31296);return null;}
async function findOrCreateDriveFolderV31296(){
  const stored=await getStoredDriveFolderV31296();if(stored)return stored;
  const q=`name = '${DRIVE_BACKUP_FOLDER_V31296.replace(/'/g,"\\'")}' and mimeType = 'application/vnd.google-apps.folder' and trashed = false`;
  try{const r=await driveFetchV31296(`https://www.googleapis.com/drive/v3/files?spaces=drive&pageSize=10&fields=files(id,name,mimeType)&q=${encodeURIComponent(q)}`);const data=await r.json();const f=data.files?.[0];if(f){localStorage.setItem(DRIVE_FOLDER_ID_KEY_V31296,f.id);return f;}}catch(_){ }
...
function initDriveBackupButtonV31296(){
  ...
  driveButton.addEventListener('click',async()=>{
    ...
    try{
      ...
      await requestDriveAccessTokenV31296(true);
      const folder=await findOrCreateDriveFolderV31296();const backup=await buildCompleteBackupPayloadV31245();const fileName=`Oraciones_V3_backup_${driveStampV31296()}.json`;await uploadBackupToDriveV31296(folder.id,fileName,backup);
      const now=new Date().toISOString();localStorage.setItem(DRIVE_LAST_BACKUP_KEY_V31296,now);setBackupPendingV31268(false);saveBackupStatusV3149('Copia en Google Drive',fileName);refreshDriveBackupStatusV31296();toast('Copia guardada en Google Drive');
...
let pendingServiceWorkerV31297=null;
function showUpdateNoticeV31297(worker){
  if(worker) pendingServiceWorkerV31297=worker;
  if(updateNoticeShownV31297)return;
  updateNoticeShownV31297=true;
  let modal=document.getElementById('updateModalV31297');
  if(!modal){
    modal=document.createElement('div');
    modal.id='updateModalV31297';
    modal.className='update-modal-v31296';
    modal.innerHTML='<div class="update-sheet-v31296" role="dialog" aria-modal="true"><h2>Nueva actualización disponible</h2><p>Hay una nueva versión de Oraciones preparada.</p><div class="update-actions-v31296"><button id="updateLaterV31297" class="btn soft" type="button">Más tarde</button><button id="updateNowV31297" class="btn primary" type="button">Actualizar ahora</button></div></div>';
    document.body.appendChild(modal);
  }
  modal.classList.remove('hidden');
  document.getElementById('updateLaterV31297').onclick=()=>{modal.classList.add('hidden');updateNoticeShownV31297=false;};
  document.getElementById('updateNowV31297').onclick=()=>{
    const waiting=pendingServiceWorkerV31297;
    if(waiting) waiting.postMessage({type:'SKIP_WAITING'});
    else location.reload();
  };
}
(function setupUpdateNoticeV31297(){
  ...
  window.addEventListener('load',async()=>{
    try{
      const reg=await navigator.serviceWorker.register('sw.js?v=3.1.320',{updateViaCache:'none'});
      ...
      reg.addEventListener('updatefound',()=>{
        ...
        worker.addEventListener('statechange',()=>{
          if(worker.state==='installed'&&navigator.serviceWorker.controller){
            pendingServiceWorkerV31297=worker;
            showUpdateNoticeV31297(worker);
          }
        });
      });
...
const CARD_CATEGORY_CATALOG_V31282=[
  ...
  {id:'salvacion',label:'Salvación',designs:[{style:'fe',src:'card-fe-v2219.jpg'},{style:'salvacion-2',src:'card-salvacion-2-v31282.png'},{style:'salvacion-3',src:'card-salvacion-3-v2287.png'},{style:'salvacion-4',src:'card-salvacion-4-v31301.png'}]},
  {id:'segunda-venida',label:'Segunda venida',designs:[{style:'segunda-venida',src:'card-segunda-venida-v2219.jpg'},{style:'segunda-venida-2',src:'card-segunda-venida-2-v31282.png'},{style:'segunda-venida-3',src:'card-segunda-venida-3-v31301.png'}]},
  ...
  {id:'cristo-es-dios',label:'Cristo Jesús es Dios',designs:[{style:'cristo-es-dios',src:'card-cristo-es-dios-v2230.jpg'},{style:'cristo-es-dios-2',src:'card-cristo-es-dios-2-v31282.png'},{style:'cristo-es-dios-3',src:'card-cristo-es-dios-3-v31301.png'}]},
  {id:'fe',label:'Fe',designs:[{style:'fe-nueva',src:'card-fe-nueva-v3261.png'},{style:'fe-2',src:'card-fe-2-v31282.png'},{style:'fe-3',src:'card-fe-3-v31301.png'}]},
  {id:'dios',label:'Dios',designs:[{style:'dios',src:'card-dios-v3261.png'},{style:'dios-2',src:'card-dios-2-v31282.png'},{style:'dios-3',src:'card-dios-3-v31301.png'}]}
];
...
const CARD_STYLE_CATEGORY_IDS_V31315=Object.fromEntries(CARD_CATEGORY_CATALOG_V31282.flatMap(category=>category.designs.map(design=>[design.style,category.id])));
...
function openCardCategoryV31282(categoryId){
  ...
  grid.innerHTML=category.designs.map((design,index)=>`<button class="card-design-option-v31282" type="button" onclick="chooseCardStyleV2217('${design.style}')"><img src="${design.src}?v=3.1.320" alt="Diseño ${index+1} de ${cardCategoryEscapeV31282(category.label)}"><span><strong>Diseño ${index+1}</strong><small>${cardCategoryEscapeV31282(category.label)}</small></span></button>`).join('');
}
...
async function shareVerseCard(cardStyle="classic"){
  try{
    ...
    // V3.1.304 — la referencia conserva 74 px siempre que tenga margen suficiente.
    // Si es muy larga, se reduce solo lo necesario para no acercarse al marco.
    const REFERENCE_FONT_SIZE_V3241=74;
    const REFERENCE_MIN_FONT_SIZE_V31304=46;
    const REFERENCE_SAFE_WIDTH_V31304=900;
    let referenceFontSizeV31304=REFERENCE_FONT_SIZE_V3241;
    while(referenceFontSizeV31304>REFERENCE_MIN_FONT_SIZE_V31304){
      ctx.font="bold "+referenceFontSizeV31304+"px Georgia, serif";
      if(ctx.measureText(ref).width<=REFERENCE_SAFE_WIDTH_V31304)break;
      referenceFontSizeV31304--;
    }
    ctx.font="bold "+referenceFontSizeV31304+"px Georgia, serif";
...
(function(){
  ...
  window.toggleReadingUI=function(){
    ...
    var wasAtPageTop=(root&&root.scrollTop||window.pageYOffset||0)<=48;
    document.body.classList.toggle('hide-reading-ui');
    if(!willHide || !wasAtPageTop) return;
    window.setTimeout(function(){
      try{
