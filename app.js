...
/* ===== V3.1.150 - Desplegables estables + emergente unificado ===== */
(function(){
  if(window.__v31150UnifiedPopup) return;
  window.__v31150UnifiedPopup=true;

  /* Mantiene únicamente la corrección ya confirmada para los desplegables. */
  document.addEventListener('click',function(ev){
    ...
    var content=document.querySelector('.content');
    var contentTop=content ? content.scrollTop : 0;
    details.open=!details.open;
    try{ if(content) content.scrollTop=contentTop; }catch(e){}
  ...
  function ensureOverlay(){
    ...
    overlay.innerHTML='<div class="reader-popup-card-v908" role="dialog" aria-modal="true">'+
      '<h3 class="v31150-popup-title"></h3>'+
      '<div class="reader-popup-content-v908 v31150-popup-content"></div>'+
      '<div class="reader-popup-actions-v913">'+
      '<button class="btn soft v31150-edit" type="button">✏️ Editar</button>'+
      '<button class="btn soft danger v31150-delete" type="button">🗑️ Eliminar</button>'+
      '<button class="btn primary v31150-close" type="button">Cerrar</button>'+
      '</div></div>';
    overlay.addEventListener('click',function(ev){
      if(ev.target===overlay || ev.target.closest('.v31150-close')){
        window.closeReaderPopupBlockV908();
      }else if(ev.target.closest('.v31150-edit')){
        var i=currentIndex;
        window.closeReaderPopupBlockV908();
        if(typeof window.editPopupBlockV908==='function') window.editPopupBlockV908(i);
      }else if(ev.target.closest('.v31150-delete')){
        var j=currentIndex;
        window.closeReaderPopupBlockV908();
  ...
  function install(){
    ...
    window.openReaderPopupBlockV908=function(idx){
      try{
        /* Retira el foco del botón que lanzó la apertura, sin tocar el scroll. */
        var active=document.activeElement;
        if(active && active.classList && active.classList.contains('reader-popup-title')){
        ...
        var title=(typeof escapeHtml==='function') ? escapeHtml(b.title||'Emergente') : String(b.title||'Emergente');
        var body=(typeof highlightBibleReferencesV49==='function') ? highlightBibleReferencesV49(b.body||'') : ((typeof escapeHtml==='function') ? escapeHtml(b.body||'') : String(b.body||''));
        el.querySelector('.v31150-popup-title').innerHTML=title;
        var content=el.querySelector('.v31150-popup-content');
        content.innerHTML=body;
        content.scrollTop=0;
        el.classList.add('v31148-visible');
        el.setAttribute('aria-hidden','false');
      }catch(e){
        console.error('openReaderPopupBlockV31150',e);
      }
    };
