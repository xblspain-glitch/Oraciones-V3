...
function setActiveView(view){
  ...
  const map = {
    ...
    'biblical-dictionary': 'biblicalDictionaryBtnV2264',
    random: 'btnRandom',
    titles: 'btnTitles',
    edit: 'btnEdit',
...
/* ===== V3.1.249 · Copia limpia de la aplicación ===== */
const INSTALLED_APP_FILES_V31249 = ["index.html", "app.js", "styles.css", "themes.css", "welcome.js", "config.js", "utils.js", "recent.js", "versiculos.js", "theme-mode.js", "jszip.min.js", "patches.js", "routines.js", "moments.js", "counters-v3183.js", "sw.js", "manifest.json", "biblical-characters-v2243.css", "biblical-characters-v2243.js", "biblical-characters-v2261.json", "biblical-dictionary-v2264.css", "biblical-dictionary-v2264.js", "biblical-dictionary-v2264.json", "cross-ethiopian-mask.png", "icon-notas-detallado-v2210.png", "icon-guia-detallado-v2210.png", "icon-personajes-biblicos-v2255.png", "icon-192.png", "icon-512.png", "bg-morning.webp", "bg-day.webp", "bg-sunset.webp", "bg-night.webp", "card-sabiduria-v2240.jpg", "routine-morning-bible-v2216.webp", "routine-night-bible-v2216.webp", "shared-card-new-jerusalem-v2217.png", "card-salvacion-v2219.jpg", "card-oracion-v2219.jpg", "card-espiritu-santo-v2219.jpg", "card-misericordia-v2219.jpg", "card-alabanza-v2219.jpg", "card-fortaleza-v2219.jpg", "card-amor-v2219.jpg", "card-esperanza-v2219.jpg", "card-juicio-v2219.jpg", "card-fe-v2219.jpg", "card-segunda-venida-v2219.jpg", "card-reino-dios-v2230.jpg", "card-santidad-v2230.jpg", "card-cristo-es-dios-v2230.jpg"];
async function readInstalledAppFileV31249(fileName){const clean=String(fileName||"").replace(/^\.\//,"");const url=new URL(clean,document.baseURI).href;if("caches" in window){try{const x=await caches.match(url,{ignoreSearch:true});if(x&&x.ok)return await x.arrayBuffer()}catch(e){}}const x=await fetch(url,{cache:"no-store",credentials:"same-origin"});if(!x.ok)throw new Error(clean+" · HTTP "+x.status);return await x.arrayBuffer()}
async function exportInstalledAppZipV31249(){const filename="oraciones_v3_1_249_copia_app.zip";try{if(typeof window.JSZip==="undefined"&&typeof JSZip==="undefined")throw new Error("JSZip no está disponible");toast("Preparando copia limpia de la aplicación…");const zip=new (window.JSZip||JSZip)(),missing=[];for(let i=0;i<INSTALLED_APP_FILES_V31249.length;i++){const f=INSTALLED_APP_FILES_V31249[i];try{zip.file(f,await readInstalledAppFileV31249(f))}catch(e){missing.push(f)}}if(missing.length)throw new Error("Faltan recursos utilizados: "+missing.join(", "));zip.file("INFORMACION_COPIA_APP.json",JSON.stringify({type:"oraciones-v3-copia-aplicacion",appVersion:"3.1.249",exportedAt:new Date().toISOString(),includedFiles:INSTALLED_APP_FILES_V31249,complete:true,futureIcons:["icon-versiculo-dia-v31249.png","icon-diccionario-v31249.png"]},null,2));zip.file("LEEME_COPIA_APP.txt","ORACIONES V3 · COPIA LIMPIA

Versión: 3.1.249

Incluye solo los recursos activos. Los dos iconos propios quedan preparados, pero no se incluyen hasta que existan.
");const blob=await zip.generateAsync({type:"blob",compression:"DEFLATE",compressionOptions:{level:6}});if(!downloadBlob(filename,blob))throw new Error("No se pudo iniciar la descarga");saveBackupStatusV3149("Copia limpia de la aplicación",filename);toast("Copia completa de la aplicación descargada")}catch(e){console.error(e);alert("No se pudo descargar la copia de la aplicación: "+(e&&e.message?e.message:"error desconocido"))}}
...
const COMPLETE_BACKUP_VERSION_V31245 = 31247;
